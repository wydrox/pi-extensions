...
export default function (pi: ExtensionAPI) {
	pi.registerFlag("minimal-mode", {
		description: "Enable minimal-mode tool rendering overrides",
		type: "boolean",
		default: false,
	});

	// This is a demo extension that overrides core tools, so keep it opt-in.
	// Without this guard it conflicts with other extensions that also wrap tools.
	const isEnabled = process.argv.includes("--minimal-mode");
	if (!isEnabled) return;
